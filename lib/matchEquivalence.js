...
export function applyMarkingFees(processedLines, config = {}) {
  ...
  const hasAnyMarquage = markedLines.length > 0;

  // Calculer le total de pièces marquées par préfixe (= par taille de cône)
  const qtyByPrefix = {};
  ...
  // On les ajoute en fin de tableau, groupés par préfixe
  if (refFraisGestion) {
  ...
  // Frais d'écran : UNE SEULE FOIS par commande si marquage présent
