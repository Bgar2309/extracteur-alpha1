 ...
 * 1. Match exact par clé (référence Prozon numérique)
 * 2. Match normalisé par clé
 * 3. Match par ehsRef1 (libellé EHS utilisé à la place de la ref numérique)
 * 4. Match normalisé par ehsRef1
 *
...
export function matchReference(prozonRef, equivalenceMap) {
  ...
  // 1. Exact match par clé (référence numérique Prozon)
  ...
  // 3. Match sans zéros initiaux (ex: "02325" dans le BL → "2325" dans le JSON)
  const keyNoLeadingZeros = keyExact.replace(/^0+/, '');
  if (keyNoLeadingZeros && equivalenceMap[keyNoLeadingZeros]) {
    return { found: true, entry: equivalenceMap[keyNoLeadingZeros], matchType: 'no_leading_zeros' };
  }

  // 4. Match par ehsRef1 (Prozon peut utiliser le libellé EHS à la place de sa propre ref)
  for (const [, v] of Object.entries(equivalenceMap)) {
    if (v.ehsRef1 && v.ehsRef1.toLowerCase() === keyExact) {
      return { found: true, entry: v, matchType: 'ehsRef1_exact' };
    }
  }

  // 4. Match normalisé par ehsRef1
  for (const [, v] of Object.entries(equivalenceMap)) {
    if (v.ehsRef1 && normalizeRef(v.ehsRef1) === keyNorm) {
      return { found: true, entry: v, matchType: 'ehsRef1_normalized' };
    }
  }
  ...
  for (const line of rawLines) {
    ...
    if (!found || !entry) {
      ...
      processed.push({
        ...
        ehsRef1: null,
        ehsRef2: null,
        // FIX #2 : quand non trouvé, on garde la désignation Prozon telle quelle
    ...
    // FIX #2 : désignation EHS = ehsRef1 (libellé EHS propre), PAS la désignation Prozon
    // La désignation Prozon (longue, avec options) est gardée dans designation_prozon
    const designation_ehs_1 = entry.ehsRef1 || line.designation;

    // Détermine si le produit est un cône (pour l'application des frais de gestion/écran)
    // On vérifie que la désignation Prozon dans la table commence par "Cône"
    const isCone = /^c[oô]ne/i.test(entry.designation || '');
    ...
    processed.push({
      ...
      // FIX #2 : on utilise ehsRef1 comme désignation affichée côté EHS
      designation_ehs: designation_ehs_1,
    ...
    // On ignore ehsRef2 quand c'est du marquage — applyMarkingFees génère cette ligne
    const ehsRef2IsMarquage = entry.ehsRef2 &&
      /marquage|s\u00e9rigraphie|serigraphie|impression/i.test(entry.ehsRef2);

    if (entry.ehsRef2 && !ehsRef2IsMarquage) {
      processed.push({
        ref_prozon: line.ref_prozon,
        designation_prozon: line.designation,
        ehsRef: entry.ehsRef2,
        // FIX #2 : désignation du composant secondaire = ehsRef2
        designation_ehs: entry.ehsRef2,
        quantite: line.quantite,
        prixPdf: null,
        prixTable: null,
        prixMatch: null,
...
export function applyMarkingFees(processedLines, config = {}) {
  ...
  // Extraire le préfixe d'une ref Prozon (avant le tiret) = identifiant de taille/famille
  // Ex: "01277-40383" → "01277", "02325" → "02325" (ref sans tiret = son propre groupe)
  const getPrefix = (ref) => {
    if (!ref) return '__unknown__';
    const idx = ref.indexOf('-');
    return idx > 0 ? ref.substring(0, idx) : ref;
  };

  // Lignes marquées éligibles (produit principal uniquement, pas composant secondaire)
  const markedLines = processedLines.filter(
    l => l.marquage && l.status === 'found' && l.composantIndex !== 2
  );

  // Frais de gestion et d'écran uniquement pour les cônes marqués
  const markedConeLines = markedLines.filter(l => l.isCone);
  const hasConeMarquage = markedConeLines.length > 0;

  // Calculer le total de pièces marquées par préfixe (= par taille de cône), cônes uniquement
  const qtyByPrefix = {};
  for (const line of markedConeLines) {
    const prefix = getPrefix(line.ref_prozon);
    qtyByPrefix[prefix] = (qtyByPrefix[prefix] || 0) + (line.quantite || 0);
  }

  // Construire le résultat : pour chaque ligne, insérer les frais de marquage juste après
  const result = [];
  ...
  for (const line of processedLines) {
    ...
    if (line.marquage && line.status === 'found' && line.composantIndex !== 2) {
      // Frais de sérigraphie (un par ligne produit, quantité = celle de la ligne)
      if (refMarquage) {
        result.push({
          ref_prozon: null,
  ...
  // Frais de gestion : UN PAR TAILLE (préfixe) si total pièces de cette taille < seuil
  // Uniquement pour les cônes marqués
  if (refFraisGestion) {
    const prefixesDejaTreites = new Set();
    for (const line of markedConeLines) {
      const prefix = getPrefix(line.ref_prozon);
      if (!prefixesDejaTreites.has(prefix)) {
        prefixesDejaTreites.add(prefix);
        if (qtyByPrefix[prefix] < seuilFraisGestion) {
          result.push({
            ref_prozon: null,
            designation_prozon: null,
  ...
  // Frais d'écran : UNE SEULE FOIS par commande si au moins un cône marqué
  if (inclureFraisEcran && refFraisEcran && hasConeMarquage) {
    result.push({
      ref_prozon: null,
      designation_prozon: null,
...
export function generateOdooOutput(header, blData, processedLines) {
  ...
  lines.push('Ref EHS\tDésignation EHS\tQuantité\tPrix unitaire\tTotal');
  for (const line of processedLines) {
